...
from sparql_ply.components import TriplesPath, Union, Query, GraphPattern, QueryComponent, NodeTerm
...
def analysis_temporal_taxonomy_sparql(sparql):
    ...
    def convert_triple_into_nary_facts(triples):
        #将parser格式的triple pattern先转成标准三元组，再根据statement节点修改成若干n元事实
    ...
    #我们可以解析SPARQL来获取问题对应的时间约束
    #我们需要做的：1、将三元组重新划分为事实。2、提取各FILTER。3、提取solution modifier。
    try:
        ...
        parse_tree = None
        return None
    #我们收集所有的三元组triples并转换为facts，同时收集solution modifier，bind，filter
...
def word_count():
    data = list()
    for file in ['train', 'valid', 'test']:
        data += json.load(open(f'json/{file}.json', 'r'), strict=False)
    word_count_dict = dict()
    for example in data:
        words = len(example['paraphrases'][0].strip().split(' '))
        if words not in word_count_dict.keys():
    ...
    for key in word_count_dict.keys():
        sum += word_count_dict[key]
        total_words += key * word_count_dict[key]
    print(f"average: {total_words / sum}")
    word_count_keys = sorted(word_count_dict.keys())
    for i in range(word_count_keys[0], word_count_keys[-1]):
        if i not in word_count_dict.keys():
            word_count_dict[i] = 0
    x = sorted(word_count_dict.keys())
    y = [word_count_dict[key] / sum * 100 for key in x]
    print(x)
    print(y)
    plt.plot(x, y, 'b*--')
...
def analysis_split_complexity(data_dir, extraction_data_path, table_output_path):
    ...
    if 'tempqa' in data_dir:
        ...
        for split in ['test', 'train', 'dev']:
            ...
            for id in total_ids:
                if '|' in id:
                    entity_comp_ids.append(id)
                elif 'comp' in id:
                    if 'temp' in id:
                        temp_comp_ids.append(id)
                    elif 'liter' in id:
                        literal_comp_ids.append(id)
                    else:
                        print(id)
                        assert(0)
                elif "|" not in id and "comp" not in id and not re.fullmatch("\d+_\d+", id):
                    generalized_ids.append(id)
                else:
                    seed_ids.append(id)
        compositional_ids = temp_comp_ids + entity_comp_ids + literal_comp_ids
...
def calculate_splits_statics(data_dir):
    ...
    for split in ['train', 'dev', 'test']:
        ...
        print(split, 
            ...
            "static entity augmentation", len(entity_comp_ids),
            "temporal entity augmentation", len(temp_comp_ids),
            "event time augmentation", len(literal_comp_ids), 
            "total_questions", len(generalized_ids) + len(entity_comp_ids) + len(temp_comp_ids) + len(literal_comp_ids) + len (seed_ids)
        )
        total_seeds += len(seed_ids)
        total_generalized += len(generalized_ids)
        total_literal_comp += len(literal_comp_ids)
        total_entity_comp += len(entity_comp_ids)
        total_temp_comp += len(temp_comp_ids)
    print("------------------------------------------------------------------------")
    print('total', 
        "seed", total_seeds,
